    ...
    public class VenueStorage
    {
        ...
        public async Task<bool> InsertDocs(IEnumerable<Venue> venues, TraceWriter log)
        {
            ...
            try
            {
                ...
                {
                    //// TODO: Change to use faster Upsert (which currently errors as doesn't like using VENUE_ID as PartitionKey)
                    //// Any docs with this VENUE_ID already in the database? Then delete them before re-adding the provider.
                    //IEnumerable<Document> docs = docClient.CreateDocumentQuery<Document>(Collection.SelfLink,
                    //                                        new SqlQuerySpec("SELECT * FROM venues v WHERE v.VENUE_ID = @VENUE_ID",
                    //                                                         new SqlParameterCollection(new[] {
                    //                                                                    new SqlParameter { Name = "@VENUE_ID", Value = v.VENUE_ID }
                    //                                                         })),
        ...
        public async Task<IEnumerable<Venue>> GetAll(TraceWriter log)
        {
            ...
            Task<FeedResponse<dynamic>> task = docClient.ReadDocumentFeedAsync(Collection.SelfLink, new FeedOptions { MaxItemCount = -1 });
